...
async function fetchMissing() {
  const response = await fetch(
    'https://raw.githubusercontent.com/baofinance/tokenlists/main/bsc.json'
  )
  const data = await response.json()
  const bridgedTokens = [...new Set(data.result.map(x => x.contractAddress.toLowerCase()))]

  const response2 = await fetch('https://raw.githubusercontent.com/baofinance/tokenlists/main/bsc.json')
